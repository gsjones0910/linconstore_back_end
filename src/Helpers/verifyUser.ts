        ...
        <body>
            <div class="container">
                <table cellpadding="0" cellspacing="0" border="0" width="100%" >
                    ...
                    <tr>
                        <td align="center" style="padding: 20px 0;"><p>
                         Don't recognise this activity? contact us immediately</p>
                        </td>
                    </tr>
                    <tr>
                        <td align="center" style="padding: 20px 0; font-size:15px; ">
...
export const UpdateIdentityVerification = async (email: string, storeName: string) => {
    const mes = {
        to: email,
        from,
        subject: 'Action Required: Update Verification Document',
        html: `
...
export const sellerOrderReceived = async (product: any, email: string | undefined, shippingType: string, shippingAmount: number) => {
    const symbol = (exchangeCurrency.find(c => c.label === product.productId.owner.currency.toLowerCase()))?.symbol ?? "$";
    const mes = {
        to: email,
        from,
                    ...
                    <table cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width: 300px; margin: 0 auto;">
                        <tr>
                            ...
                            <td align="left" style="padding: 10px;">
                                <sub style="font-size: 10px; text-align: left;"> Product ID: ${product.productId._id}</sub>
...
export const updateOrderShippedNotification = async (trackingId: string, shipper: string, address: string, user: any, product: any, variants: TVariant[], shippingType: string, shippingAmount: number, date: Date, quantity: number) => {
    const symbol = (exchangeCurrency.find(c => c.label === user.currency.toLowerCase()))?.symbol ?? "$";
...
export const orderDeliveredSuccessfully = async (trackingId: string, shipper: string, address: string, user: any, product: any, variants: TVariant[], shippingType: string, shippingAmount: number, date: Date, quantity: number) => {
    const symbol = (exchangeCurrency.find(c => c.label === user.currency.toLowerCase()))?.symbol ?? "$";
    const userRate = await calculateRate(user?.currency?.toLowerCase() ?? "usd")
    const productRate = await calculateRate((product?.owner.currency)?.toLowerCase() ?? "usd")
