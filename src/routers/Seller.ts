...
import { Wallet } from "../components/wallet/models/wallet";
...
router.get("/seller/funds", sellerAuth, async (req: Request, res: Response) => {
  try {
    ...
    const wallet = await Wallet.findOne({ ownerId: seller?.storeId })

    res.status(200).send({
      pendingPayout: wallet?.pendingPayout ?? 0,
      availablePayout: wallet?.balance ?? 0,
    });
  } catch (error) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const countryLocation = CountryCodes.find(
        (x) => x.country === req.seller?.location
      );
